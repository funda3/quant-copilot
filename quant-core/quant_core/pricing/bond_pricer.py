...
def solve_bond_ytm(
    ...
    Finds a flat annual yield *y* such that::
        ...
        price_bond(bond, curve_y).dirty_price == market_dirty_price

    where ``curve_y`` is a flat curve defined directly on the bond's actual
    remaining payment dates (strictly after ``bond.valuation_date``).
    ...
    coupon_frequency : str | None, optional
        Retained for backward compatibility.  Ignored by the solver because
        pillars are anchored to the bond's remaining payment dates.
    tol : float, optional
    ...
    # Build a pillar set that exactly matches the bond cashflow dates so
    # seasoned bonds never request discount factors before the first pillar.
    if bond.coupon_rate == 0.0:
        _remaining_payment_dates = [bond.maturity_date]
    else:
        _remaining_payment_dates = [
            d
            for d in _coupon_schedule(
                bond.issue_date,
                bond.maturity_date,
                bond.coupon_frequency,
            )
            if d > bond.valuation_date
        ]

    # Defensive guard: valuation_date == maturity_date cannot occur here due
    # FixedRateBond validation, but keep this explicit for robustness.
    if not _remaining_payment_dates:
        raise ValueError(
            "No remaining payment dates available to solve bond YTM; "
            f"valuation_date={bond.valuation_date}, maturity_date={bond.maturity_date}."
        )

    def _price_at_yield(y: float) -> float:
        dfs = [
            1.0 / (1.0 + y * accrual_fraction(bond.valuation_date, d, _dc))
            for d in _remaining_payment_dates
        ]
        curve = DiscountCurve(bond.valuation_date, _remaining_payment_dates, dfs)
    ...
    raise ValueError(
        f"solve_bond_ytm failed to converge after {max_iter} iterations. "
        f"face_value={bond.face_value}, coupon_rate={bond.coupon_rate}, "
        f"maturity_date={bond.maturity_date}, settlement_date={bond.valuation_date}, "
        f"market_dirty_price={market_dirty_price}, "
        f"bracket=[{low:.8f}, {high:.8f}]."
    )
